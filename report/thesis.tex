...
errors matter for downstream use. Dilating the predicted mask by one pixel
before image stacking would cover most boundary-placement residuals, so the
strict-precision gap may matter less for masking; this effect remains untested
here. The classifier gate is best treated as a compute feature:
it skips $24.7\,\%$ of U-Net forwards, and likely more under the full-image
patch distribution, with precision headroom capped by the $17.5\,\%$
...
discrepancies as well as confirm headline results. Here, reconstructing the
architecture from the released Keras model, locking model and threshold
selection before test evaluation, and keeping sampled and parity evaluations
separate made the precision shortfall interpretable rather than leaving it as
an unexplained reproduction gap. Analysing that gap directly converted a
headline deficit into a measured, bounded mechanism. At this level of control,
the comparison becomes a way to locate where a published result's accuracy
resides: in the detector, in the labels, or in the metric.
...
a direct estimate of how much of the strict precision gap is mask granularity
rather than detector error. A smaller mask-cleaning experiment would test
component-level choices, such as removing isolated small objects before
training and evaluation, while treating the cleaned masks as a new target
rather than a correction to the locked results.
