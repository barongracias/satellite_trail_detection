...
def make_montage(processed: list[ProcessedImage], pdf_path: str | Path, svg_path: str | Path) -> None:
    ...
    for ax, item in zip(axes_arr, processed):
        image = rotate_panel_for_display(resize_for_display(item.image_u8, max_dim=1500, nearest=False))
        binary = rotate_panel_for_display(
            resize_for_display(item.binary_canvas.astype(np.uint8), max_dim=1500, nearest=True)
        ) > 0
        hough = rotate_panel_for_display(
            resize_for_display(item.hough_canvas.astype(np.uint8), max_dim=1500, nearest=True)
        ) > 0
