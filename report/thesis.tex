...
\abstract{%
  ...
  A single-author re-annotation of $64$ sealed crops finds no systematic
  width bias between the original masks, the re-annotation, and the model.
  Model, training, and inference variants move the residual only slightly.
...
remaining a single-author between-source check rather than a true-mask oracle.
...
gate suppresses only empty-patch false positives, so its gain is bounded by the
...
Section~\ref{sec:ext_error} localises much of the residual to boundary-scale
...
The Hough comparison shows comparable completeness while exposing lower strict
...
sensitive to the $3$\,px line-drawing convention in a line-thickness sweep
...
same caution: the $\pm1$\,px precision $0.946$ locates the residual at the
boundary scale and is kept separate from the paper's strict $0.94/0.94$, and
these diagnostics hold across the five seeds (Figure~\ref{fig:supp_multiseed}).
...
boundary, no tested change resolves that residual. The re-annotation
(Section~\ref{sec:ext_reannotation}) is consistent with the interpretation that
strict performance is limited by annotation granularity. Evaluations of
...
threshold locked on validation data alone. The replication reproduces the
paper's recall closely (five-seed sampled-test recall $0.923\pm0.007$ against
$0.94$, Dice $0.884\pm0.004$) and its Hough completeness (trail-pixel recall
$0.918\rightarrow0.990$ on the full-coverage parity canvas). Strict pixel
precision sits below the published level ($0.847$ sampled, $0.780$ at parity)
and depends on the test-set class balance, leaving recall as the like-for-like
comparison across test populations.
...
Hough - are either small or null in the tested settings. The null variants
...
residual materially, so boundary-placement remains the most consistent reading.
...
Evaluations of thin artefact detectors should therefore keep strict scores,
tolerant scores, and annotation conventions visible together.
...
annotators would establish inter-annotator agreement and turn the
single-author between-source estimate (Section~\ref{sec:ext_reannotation}) into
a direct estimate of how much mask granularity contributes to the strict
precision gap. A smaller mask-cleaning experiment would test component-level
...
application here is qualitative only (Figure~\ref{fig:decam_cold_montage}) -
paired with a classical baseline following \citet{serrano2026}. Attention
...
satellite trails. Trail \emph{removal} is the natural downstream step: the
