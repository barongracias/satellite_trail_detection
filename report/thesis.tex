...
% Handbook basis ~6,845 with section headings excluded (texcount body ~6,490
...
even that spans different populations---32 of our images against the paper's
...
cuts strict pixel precision from $0.780$ to $0.410$, about half of which a
line-thickness sweep attributes to the $3$\,px rasterisation convention rather
than genuine false detection (Figure~\ref{fig:supp_hough_thickness}). On this
full-coverage canvas, verified pixel-for-pixel against dense full-frame
inference, this is comparable completeness, not superiority. The same discipline
...
\begin{itemize}
  \item \textbf{Mask undercapture.} The annotations occasionally undercover
        ...
        bound where the labels undercover. The blinded re-annotation
  ...
  \item \textbf{Hough rasterisation convention.} The parity Hough canvas is a
        verified dense full-frame prediction, not a patch-sampling artefact; the
...
and the Hough stage makes this concrete: it extrapolates linear extent the
network under-detects, but where the annotation also undercovers that extent the
recovered pixels score as false positives---the metric penalises exactly the
recovery the stage was built for. Reported precision for this detector family
should therefore be read alongside a statement of label quality; comparisons
across papers with different annotation conventions conflate detector quality
with labelling convention.
...
The dominant residual sits within about one pixel of the labelled boundary, no
tested change moves it, and the blinded re-annotation
(Section~\ref{sec:ext_reannotation}) supports the reading that strict performance is limited by
annotation granularity. Evaluations of thin-artefact detectors should therefore
...
closely (five-seed sampled-test recall $0.923\pm0.006$ against $0.94$, Dice
...
with no systematic width bias and no detectable strict error beyond the measured
between-source labelling floor. Every tested intervention---classifier gate,
attention U-Net, data efficiency, hard-negative mining, soft labels, ensembling,
and probability-stratified Hough---is bounded or null, exactly as the error
decomposition predicts; none moves the residual materially, and that is what
makes the boundary-placement diagnosis credible.
...
Several directions follow. The decisive next check is a \emph{multi-annotator
gold-standard audit}: independent annotators would establish inter-annotator
agreement and convert the single-author labelling floor
