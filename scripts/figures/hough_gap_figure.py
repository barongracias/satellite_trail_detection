...
from scripts.figures._locked_winner_canvases import (
    LOCKED_CHECKPOINT,
    LOCKED_THRESHOLD,
...
from scripts.figures.make_thesis_figures import configure_style, save_vector
from src.utils.imaging import largest_component_mask
...
def _recovered_crop(recovered: np.ndarray, fallback: np.ndarray, raw_shape: tuple[int, int], margin: int) -> tuple[slice, slice]:
    ...
    comp = largest_component_mask(seed if seed.any() else (recovered if recovered.any() else fallback))
