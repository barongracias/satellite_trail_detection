...
def _figure_full_image_heatmap(
    ...
    raw_d = resize_for_display(raw, nearest=False)
    target_d = resize_for_display(target_canvas.astype(np.uint8), nearest=True) > 0
    binary_d = resize_for_display(binary.astype(np.uint8), nearest=True) > 0
    hough_d = resize_for_display(hough.astype(np.uint8), nearest=True) > 0
