...
\abstract{%
  ...
  width bias between the original masks, the re-annotation, and the model. The
  model, training, and inference variants move the residual only slightly, so on
  this dataset the remaining gap is unlikely to stem from model capacity,
  training protocol, or Hough post-processing alone.
}
...
test image; an independent native dense full-frame pass reproduced that parity
canvas pixel-for-pixel. The canvas is thresholded at $t_{\mathrm{Hough}}=0.1$,
...
(five-seed mean $0.077\rightarrow0.0099$). This pixel-completeness rate is the
quantity comparable to the paper's reported $6.98\,\%\rightarrow3.38\,\%$, and
the independent dense full-frame check shows it is not a patch-sampling or seam
...
counts trail-bearing patches with no recovered pixel.
...
(Table~\ref{tab:reannotation}). A paired per-crop bootstrap ($B=10{,}000$,
equal weight) puts the strict model-minus-consortium difference against the
re-annotation at $-0.0001$ ($95\%$ CI $[-0.024,+0.022]$). At $\pm1$\,px the
...
additional strict error beyond the measured between-source labelling floor. The
...
overlays. The audit remains bounded by its design: a single, dataset-familiar
annotator estimates a labelling floor rather than inter-annotator agreement, so
a multi-annotator gold standard remains the decisive next check
...
the full-coverage parity canvas, a pixel-level false-negative rate of
...
it: $82.5\,\%$ of false-positive pixels lie inside trail-bearing patches, a
one-pixel boundary tolerance raises five-seed precision from $0.847$ to $0.946$
($F_1$ $0.962$), and $57\,\%$ of all false positives lie within one pixel of a
labelled trail. A blinded single-author re-annotation then tests the main
ambiguity in that diagnosis: the original labels, the re-annotation, and the
model all have median width $6$\,px, with no systematic width bias and no
detectable strict error beyond the measured single-author labelling floor. The
variants behave as the decomposition predicts: the classifier gate shifts
precision $+1.2$\,pp, bounded by the $17.5\,\%$ inter-patch share; the
attention-gated U-Net is a null at matched capacity, with clDice confirming no
hidden topology advantage; validation Dice plateaus beyond roughly $70\,\%$ of
the training images; hard-negative mining adds $+0.007$ precision; the soft-label
pilot improves no overlap, precision or topology metric; and ensembling with
test-time augmentation and probability-stratified Hough sit within seed noise.
The residual error is bounded by the error decomposition and the re-annotation:
none of the tested architecture, training-protocol, or inference changes moves
it materially, and these bounded and null outcomes are what make the
boundary-placement diagnosis credible.
...
deficit into a measured, bounded mechanism: one-pixel boundary disagreement, no
systematic width bias in the blinded single-author audit, and a small
reference-relative over-firing control. Conducted at this level of fidelity,
...
the model near the measured single-author labelling floor. The decisive remaining check is a
...
\begin{figure}[htbp]
  ...
  \caption{Blinded re-annotation overlays for five audit crops. Contours show
           ...
           controls with model-only over-firing. The reference is a
           single-author re-annotation, not an independent gold standard.}
