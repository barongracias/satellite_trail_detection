...
post-processing is evaluated on a full-coverage canvas that reproduces native
dense full-image inference pixel-for-pixel, with separate precision and
...
dense full-frame canvas, trail-pixel recall rises from $0.918$ to $0.990$,
...
being concentrated near labelled trail boundaries. A blinded single-author
re-annotation of $64$ sealed crops bears this out: the original masks, a
blinded single-author re-annotation, and the model all have a median width of $6$\,px, and
the model agrees with the re-annotation about as well as the original labels do,
with no clear strict-$F_1$ difference (paired $-0.0001$, $95\%$ CI spanning
zero) --- the residual sits near the single-author labelling floor, not in a
thin label or a model over-paint.
...
A blinded single-author re-annotation has been completed and places the model
near the measured single-author labelling floor; the decisive remaining check is
a multi-annotator gold-standard audit, to establish inter-annotator agreement. A labelled cross-instrument test,
