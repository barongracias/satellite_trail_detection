...
% Current handbook-style rounded count: 6,920.
\wordcount{6920}
...
%%%%%
\abstract{%
  The proliferation of low-Earth-orbit satellite constellations increasingly
  contaminates wide-field survey images with linear artefacts. Automated
  satellite-trail detection and masking are therefore becoming a routine
  ...
  %
  The replication reproduces the paper's recall, with five-seed test recall
  $0.923$ against $0.94$. Hough post-processing also reproduces the paper's
  ...
  the paper's $6.98\,\%\rightarrow3.38\,\%$. Strict pixel precision remains below
  the paper and is prevalence-dependent.
  %
  The residual-error analysis localises much of the gap near annotated trails:
  $82.5\,\%$ of false positives fall in trail-bearing patches, and separate
  tolerance and distance diagnostics place much of the residual within one pixel
  of the mask. A single-author re-annotation of $64$ sealed crops does not
  support systematic width bias between the original masks, the re-annotation,
  and the model. Model, training, and inference variants do not materially close
  the strict-precision residual.
  Taken together, the evidence supports, but does not prove, a narrow
  interpretation: the remaining strict-precision gap is most consistent with
  boundary-scale annotation disagreement on this dataset. The practical
  implication is that high-completeness masking can still be useful when strict
  pixel precision is boundary-limited, and that thin-artefact detectors should
  report strict scores alongside boundary-tolerant diagnostics and mask-width
  conventions.
}
...
weak linear evidence. The published pipeline reaches strict pixel precision and
recall both approaching $0.94$ and was applied at scale to around
...
20{,}000-patch test set at threshold $0.58$ the pipeline reported strict
...
file, not the paper's text alone, and with model and threshold selection locked
on validation data before any test metric is inspected. The second is to
diagnose the residual error of that replication, asking how much of the strict
precision gap can be explained by evaluation population, boundary sensitivity,
and remaining false positives. The third is to test which interventions can
move that residual, through model variants, training changes, and
...
\begin{itemize}
  \item an architecture-faithful, validation-locked replication that
        ...
        (Chapter~\ref{ch:analysis});
  \item a residual-error characterisation that places much of the strict precision
        residual at the one-pixel boundary scale and motivates explicit reporting
        of mask-width conventions and boundary-tolerant diagnostics for thin
...
Filename-derived observation dates span 2017--2023, mostly 2022; 
because exposures and observing conditions are not normalised, these
...
near $6$\,px, estimated from distances to the nearest mask edge, consistent with
...
are to one-pixel boundary placement; interpretations are phrased as `consistent
...
\begin{figure}[htbp]
  ...
  \caption{Mask-quality inspection grid for three of the densest patches:
           ...
           endpoints, examined quantitatively in Section~\ref{sec:ext_boundary}.}
...
high-completeness masking. The remaining methodological
choices follow this: the U-Net supplies pixel-level segmentation, the
BCE--Dice loss handles severe foreground imbalance, and evaluation protocols
separate selection, and label-boundary effects.
...
configuration is locked. Sampled and parity test metrics are kept distinct: the
...
inspected}. The fixed tie-breaker, applied when means fall within
...
below the segmentation threshold. Its role is completeness, with strict precision 
...
and composed at inference, so the gate changes the locked pipeline without
...
failures. Residual GPU non-determinism is characterised through five-seed
...
so each run can be reconstructed from its configuration, seed, and recorded
environment rather than treated as bit-identical.
...
matters for high-completeness masking, while strict precision sits below the
...
a high-completeness masking stage, with strict precision evaluated separately.
...
miss. This is consistent with the low-contrast behaviour
...
$0.923$ to $0.980$, and the $\pm1$\,px $F_1$ reaches $0.962$. This is a
diagnostic relaxation of the matching rule, not a strict-score reproduction.
...
far from any labelled trail. The re-annotation of
Section~\ref{sec:ext_reannotation} is consistent with this reading, while
remaining a single-author comparison between mask sources rather than a
true-mask oracle.
The tolerant score is therefore used only as a diagnostic: it relaxes the
...
Model variants are used as diagnostic checks of the error pattern, with the
locked U-Net retained as the baseline. If the precision gap were dominated by
...
(Figure~\ref{fig:two_stage_pareto}). The locked U-Net remains the baseline. The
...
from the $100\,\%$-tuned winner and each fraction is trained once, the curve is
indicative. It is not an optimised estimate at each data size. The fixed
...
obviously bind this architecture, consistent with the replication
...
near-identical $\pm1$\,px $F_1$ ($-0.001$). The boundary band mainly widens the
predictions, with no corresponding localisation gain. This is
...
gain. Given the added inference cost, the ensemble mainly serves as a robustness
...
confidence by reducing the U-Net probability map to a binary canvas.
Replacing the binary Hough input with a probability-stratified canvas has
negligible recall effect: pixel recall changes by $+0.00003$, the patch-level
false-negative rate is identical, and Hough false-positive pixels rise from
$2{,}725{,}702$ to $2{,}785{,}274$. The result suggests that the Hough limitation
is not missing probability weighting: the locked binary stage is already
recall-saturated, and probability stratification adds false-positive pixels
without measurable recall benefit.
...
\section{Methodological differences from the target paper}
...
comparison is checked on a full-coverage canvas, but strict precision remains
sensitive to the $3$\,px line-drawing convention
(Section~\ref{sec:hough_results}). The remaining differences were not
...
Section~\ref{sec:ext_error} places much of the remaining error near annotated
boundaries, consistent with boundary-scale annotation disagreement.
...
strict pixel precision from $0.780$ to $0.410$, about half of which is sensitive
...
same caution: the $\pm1$\,px precision $0.946$ shows that much of the residual is
sensitive to one-pixel boundary placement and is kept separate from the paper's
strict $0.94/0.94$. The same pattern holds across the five seeds
...
detector can recover trail extent well enough for masking, and whether strict
...
boundary, and no tested change resolves that residual. The re-annotation
(Section~\ref{sec:ext_reannotation}) is consistent with the interpretation that
part of the strict score is limited by annotation granularity. Evaluations of
thin-artefact detectors should therefore record a mask-width convention and
report a $\pm1$\,px boundary-tolerant operating point alongside the strict
...
work supports high-completeness trail masking and identifies which residual
...
the measured boundary residual; its effect on stacking, photometry, and false
...
threshold locked on validation data alone. The stable paper-facing comparisons
...
$0.990$ on the full-coverage parity canvas after Hough. Strict pixel precision
...
with no systematic width bias. The model--re-annotation strict score does not
...
explanations without making the boundary diagnosis decisive. None resolves the
residual materially, so boundary-scale annotation disagreement remains the most
consistent, but not proven, reading.

The broader methodological lesson is that faithful replication can diagnose
discrepancies as well as confirm headline results. Here, validation-locked
...
strict precision, while still requiring caution about deployment readiness.
Evaluations of thin artefact detectors should therefore report strict pixel
scores alongside boundary-tolerant scores and mask-width conventions.

\section{Future work}
\label{sec:future}

Future work should separate three remaining questions: annotation agreement,
cross-instrument generalisation, and downstream trail handling. The strongest
remaining check is a \emph{multi-annotator annotation audit}. Independent
...
of how much mask granularity contributes to the strict precision gap. A smaller
...
The cleaned masks would define a new target and leave the locked results
unchanged.
...
\begin{figure}[htbp]
  ...
  \caption{Blinded re-annotation overlays for five audit crops. Contours show
           the original MeerLICHT mask (orange), the single-author
...
\begin{figure}[htbp]
  ...
  \caption{Illustrative crop--mask pairs from the re-annotation audit.
           Left: sealed audit crops before annotation; right: exported binary
           re-annotation masks. The full private set is not redistributed.
