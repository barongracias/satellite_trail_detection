...
% Handbook basis ~6,900 with section headings excluded (texcount body 6,494
% + abstract ~175 + table cells ~300 - auto-generation appendix 75).
\wordcount{6900}
...
structures, and the densest patches adequately cover the visible
...
is penalised as a false positive, so strict label-referenced metrics are lower
bounds where labels undercover. Section~\ref{sec:ext_boundary} quantifies this
...
(fifteen each) after a preliminary free-sampling sweep concentrated 36 of 50
...
confusion. The interpretation stays provisional: no independent
...
(Section~\ref{sec:hough_results}). The rest are incidental: architecture tests
...
direct size of the base-rate effect, while the annotation confound is bounded at
one pixel by Section~\ref{sec:ext_error}. Evaluating at the paper's fixed $0.58$
...
after. The same run raises strict false-positive area, cutting strict pixel
precision from $0.780$ to $0.410$, partly because Hough lines are drawn thicker
than the $1$--$3$ px masks; with the canvas reconstructed from patches rather
than dense full-frame inference (Section~\ref{sec:hough}), this is comparable
completeness, not superiority. The same discipline applies to the tolerant
...
Several directions follow from the findings. The most decisive is a
