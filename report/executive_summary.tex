...
Pixel precision is lower, at $0.847\pm0.010$ on the sampled test set and
...
replication performance for the selected U-Net compared with Stoppa et al.\
Paper-reference markers are shown only for the explicitly reported
precision and recall values; recall is closely reproduced while pixel precision
...
$0.0105$. This does not make the Hough mask a better pixel classifier:
post-Hough line drawing increases false-positive area, reducing pixel precision
...
Diagnostics place much of the pixel-precision gap near annotated boundaries.
...
pixel of a labelled trail. These diagnostics place much of the error near
labelled trail boundaries. A blinded single-author
re-annotation of $64$ sealed crops supports this reading: the original masks,
...
with no pixel-$F_1$ difference resolved under this design (paired $-0.0001$,
$95\%$ CI spanning zero). This is a single-author comparison between mask
sources, not inter-annotator agreement. A small over-firing component remains on
...
one-pixel tolerance largely closes the pixel-precision gap, supporting a
boundary-near interpretation.}
...
test-patch predictions for the selected U-Net: ground truth in green,
prediction in magenta, and overlap inside the green band.}

\section*{Extensions \& Transfer}
Follow-up experiments test whether the precision gap can be reduced by model or
...
\vspace{0.45em}
\summaryfigure{0.95}{../results/figures/decam_cold_inference_montage.pdf}{Cold-domain
DECam examples: selected MeerLICHT-trained U-Net contours and Hough overlays on
predeclared measured-streak frames. This is a qualitative transfer illustration,
not a benchmark.}

\newpage

\section*{Recommendation \& Impact}
The practical result is a high-recall trail masker whose remaining
pixel-precision gap is largely boundary-near. The reporting standard should be
pixel precision/recall plus a boundary-tolerant operating
...
agreement and test how much of the precision gap reflects mask
...
post-processing changes do not materially reduce the remaining pixel-precision
gap. For operational masking, the evidence points to reporting pixel
