...
selection. This distinction matters because precision changes with empty-patch
prevalence, while recall over retained positive patches does not.
...
bound where masks omit visible trail pixels. Section~\ref{sec:ext_boundary}
quantifies this boundary disagreement; interpretations are therefore phrased as
`consistent with' boundary disagreement, not as proof of label error.
...
becomes a lower bound, and the Hough stage makes this concrete: it extrapolates
linear extent the network under-detects, but where the annotation also omits that
extent the recovered pixels score as false positives---the metric penalises
exactly the recovery the stage was built for. Reported precision for this
